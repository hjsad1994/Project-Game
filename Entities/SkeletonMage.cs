...
namespace Project_Game.Entities
{
    public class SkeletonMage : Enemy
    {
        ...
        public int AttackRange { get; private set; } = 200;
        ...
        private float roamSpeed = 0.8f;

        // Lưu vị trí dưới dạng float để di chuyển mượt mà
        private float posX, posY;

        public SkeletonMage(string baseFolder, int maxHealth = 50, int startX = 500, int startY = 100)
            : base("SkeletonMage", maxHealth)
        {
            Speed = roamSpeed;
            posX = startX;
            posY = startY;
            baseFolderPath = baseFolder;
            Console.WriteLine("[Debug] SkeletonMage constructor baseFolder: " + baseFolder);
        ...
        public override void PerformAttack(Player target)
        {
            if (!IsAttacking && !isDead)
            {
                ...
                if ((now - lastAttackTime).TotalMilliseconds >= attackCooldown)
                {
                    ...
                    CreateBulletFourDirections(target);

                    Console.WriteLine("SkeletonMage đã bắn đạn 4 hướng!");
                }
            }
        }
        private void CreateBulletFourDirections(Player target)
        {
            string bulletBaseFolder = Path.Combine(baseFolderPath, "Bullet");
            ...
            {
                bulletDirection = (deltaX < 0) ? "Left" : "Right";
            }
            else
            {
                bulletDirection = (deltaY < 0) ? "Up" : "Down";
            }

            // Tạo Bullet với hướng đã xác định
            Bullet bullet = new Bullet(bulletDirection, X, Y, bulletBaseFolder);
            bullet.SetMaxDistance(200);
            bullet.SetStartPosition(X, Y);

            GameObjectManager.Instance.AddGameObject(bullet);
            Console.WriteLine($"SkeletonMage bắn đạn hướng {bulletDirection} từ ({X}, {Y}) về phía Player.");
        }
        ...
        public static List<SkeletonMage> CreateEnemies(string baseFolder, int count, int startX, int startY)
        {
            var enemies = new List<SkeletonMage>();
            for (int i = 0; i < count; i++)
            {
                int x = startX + random.Next(-100, 100); // Sử dụng 'random' duy nhất
                int y = startY + random.Next(-100, 100); // Sử dụng 'random' duy nhất
                var enemy = new SkeletonMage(baseFolder, 50, x, y);
                enemies.Add(enemy);
            }
            Console.WriteLine($"[Info] Tạo {count} SkeletonMage từ thư mục '{baseFolder}' tại vị trí bắt đầu ({startX}, {startY}).");
            return enemies;
        }
